...
from skill_state import SkillState, SkillStates, SkillSimulationData
...
def calculate_skill_simulation_data(encounter, skill):
    ...
    try:
        ...
        return SkillSimulationData(damage, latest_influence_time_ms)
    except Exception as e:
        print(f"exception: {e}")
        return SkillSimulationData(0, 0)
...
def get_relevant_skills_from_encounter(encounter: Dict) -> SkillStates:
    ...
    for skill in encounter["actors"][0]["build"]["skills"]:
        if skill["skill_key"] not in allowed_skills_by_weapon[0] \
            ...
            continue
        skill_simulation_data = calculate_skill_simulation_data(encounter, skill)
...
def main():
    ...
    total_damage, best_skill_sequence = search_greedy(skill_states, 30000)
    print("--------")
